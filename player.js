...
var LoverryPlayer = (() => {
  ...
  var setBackground = (file) => {
    ...
    const img = new Image();
    img.onload = () => {
      bgLayer.style.backgroundImage = `url('/assets/background/${file}')`;
    };
    img.src = `/assets/background/${file}`;
  };
  var setCharacter = (characterId, sprite, position, hidden) => {
    ...
    img.src = `/assets/characters/${characterId}/${spriteFile}`;
    (() => {
      if (!position) {
        return;
      }
  ...
  var loader = new import_engine.SingleJsonSceneLoader("/scenes/scenes.json");
